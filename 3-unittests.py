...
class TestHW2(unittest.TestCase):
    ...
    def spin_up_nodes(self):
        ...
        for i in range(5): 
            print i
            exec_string = "docker run -p %s:%s --net=my-net --ip='%s' -e 'IP=%s' -e 'PORT=%s' -e 'MEMBERS=%s' -e PYTHONUNBUFFERED=0 -d student/hw3" % (self.host_ports[i], self.docker_internal_ports[i], self.ip_addresses[i], self.ip_addresses[i], self.docker_internal_ports[i] ,members_str)
            print exec_string
